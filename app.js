...
let products = [
  ...
  { id: 'p2', image: 'images/1.png', title: 'Huile 2',                 description: 'Bio certifié, goût fruité.',               price: 145 },
  { id: 'p3', image: 'images/3.png', title: 'Huile 3',                 description: 'Tradition familiale.',                     price: 135 },
];
let cart        = [];
let editingId   = null;
  ...
  document.getElementById('produitsDynamicContent').innerHTML = `
    ...
    <div class="form-produit" id="formWrapper" style="display:none;">
      ...
      <div class="form-group">
        <label>Image :</label>
        <input type="file" id="prodImageFile" accept="image/*" onchange="handleUpload(event)"
               style="width:100%;padding:7px 10px;border:1.5px solid var(--cream2);border-radius:var(--r);background:var(--gold2);cursor:pointer;">
...
function saveProduct() {
  ...
  if (!image || !title || !desc || isNaN(price) || price <= 0)
...
function toggleForm() {
  const isOpen = document.getElementById('formWrapper').style.display !== 'none';
